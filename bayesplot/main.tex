...
\vspace{-0.25in}
  \begin{columns}
    \begin{column}{0.03\paperwidth} % left margin space
    \end{column}
...
The pairs plot is helpful in determining if you have any highly correlated parameters, but necessitates us extracting the \texttt{stanfit} object from out \texttt{stanreg} model.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{posterior_chains} \hlkwb{<-} \hlstd{rstan}\hlopt{::}\hlkwd{extract}\hlstd{(model}\hlopt{$}\hlstd{stanfit,}
                                   \hlkwc{inc_warmup} \hlstd{=} \hlnum{TRUE}\hlstd{,}
                                   \hlkwc{permuted} \hlstd{=} \hlnum{FALSE}\hlstd{)}
...
\begin{alltt}
\hlstd{model} \hlopt{%>%}
    \hlkwd{posterior_predict}\hlstd{(}\hlkwc{draws} \hlstd{=} \hlnum{50}\hlstd{)} \hlopt{%>%}
    \hlkwd{ppc_dens_overlay}\hlstd{(}\hlkwc{y} \hlstd{= model}\hlopt{$}\hlstd{y,}
                     \hlkwc{yrep} \hlstd{= .)}
\end{alltt}
\end{kframe}
...
\begin{alltt}
\hlstd{model} \hlopt{%>%}
    \hlkwd{posterior_predict}\hlstd{(}\hlkwc{draws} \hlstd{=} \hlnum{500}\hlstd{)} \hlopt{%>%}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fargs} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlkwc{ncol} \hlstd{=} \hlnum{2}\hlstd{,}
                   \hlkwc{labeller} \hlstd{= label_parsed)}
...
\hlkwd{mcmc_trace}\hlstd{(posterior_chains,} \hlkwc{pars} \hlstd{= pars,}
           \hlkwc{n_warmup} \hlstd{=} \hlnum{300}\hlstd{,}
           \hlkwc{facet_args} \hlstd{= fargs)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{mcmc_scatter}\hlstd{(}
  \hlkwd{as.matrix}\hlstd{(model}\hlopt{$}\hlstd{stanfit),}
  \hlkwc{pars} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"wt"}\hlstd{,} \hlstr{"sigma"}\hlstd{),}
